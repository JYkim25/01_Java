...
public class ByteService {
	 ...
	 * 	 모든 것을 입/출력 할 수 있음
	 *   (단, 통로가 좁다보니 속도가 좀 느림, char형 문자의 경우 깨질 우려가 있음)
	 * 
	 * 
	 */
	...
	public void fileByteOutput() {
		...
		FileOutputStream fos = null; //11:22 전역변수 초기화 관련
		...
		try {
			...
			sb.append("~!@#$%^&*()\n");
			sb.append("가나다라마바사아\n");
			sb.append("와 자바 너무 재..재밌다..\n");
			sb.append("선생님 저 공부 정말 열심히 할게요~!\n");
			sb.append("진짜로!! 화이팅!! 와아!! 와아ㅏㅏ...\n");
			
			// 출력방법 1 : 한 글자(2byte)씩 파일로 출력
			
			// StringBuilder -> String 변환 -> charAt() -> char로 변환 *11:32
			...
			*/
			...
			fos.write(content.getBytes()); // byte[]로 만들어 반환 * 검토 필요
			...
			try {
				...
				if(fos != null) fos.close(); // close에 flush가 포함됨
	...
	public void bufferedFileByteOutput() {
		...
		try {
			...
			String content = sb.toString(); // *12:17
	 ...
	 * -> 읽어오는 파일의 크기가 작으면 보조 스트림의 성능 이점이
	 * 		크게 눈에 띄지는 않음
	 * 
	 */
	 ...
	 * -> BufferedReader 는 보조스트림으로 단독사용 불가능!
