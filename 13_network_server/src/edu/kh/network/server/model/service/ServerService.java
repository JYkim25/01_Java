...
public class ServerService {
	...
	public void serverStart() {
		...
		// 6) 보조 스트림을 통해 성능 개선
			br = new BufferedReader(new InputStreamReader(is)); //12:11
			// 문자 기반 BufferedReader와
			// 바이트 기반 InputStream을 연결할 수 있도록
			// InputStreamReader를 함께 사용
			
			pw = new PrintWriter(os); //문자기반 보조스트림이지만 os 바로 전달가능
			
			
		// 7) 스트림을 통해 읽고 쓰기
			// 서버 - 클라이언트 메세지 전송
		
			// 2025년 03 06일 12:17:30 [서버 접속 성공]
			Date now = new Date(); // java.util.Date : 자바에서 시간을 나타내는 객체
								// new Date() 객체 생성 시 현재 시간이 저장 되어있다
			// SimpleDateFormat : 날짜 형식을 간단히 지정하는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy년 MM월 DD일 HH:mm:ss");
			
			String str = sdf.format(now) + "[서버 접속 성공]"; // 12:20
