...
// ClsService와 Student/TestVo 는 패키지가 다름
public class ClsService /* extends Student */ {
			// 부모로 Student 클래스를 상속받음
			// ClsService(자식) <- Student(부모)

	//public ClsService() {} //기본 생성자 // 작성하지 않은 경우, 클래스 사용시 컴파일러가 자동으로 생성
	...
	public void ex2() {
		...
		System.out.println( std1.schoolName ); //기울어진 글씨는 static변수
	...
	public void ex3() {
		...
		User u3 = new User("test3", "password1234"); //순서가 중요!
		// 매개변수 생성자
	...
	public void ex4() {
		...
		System.out.printf("u2 : %s / %s, / %s, %d / %c\n",
				u2.getUserId(), u2.getUserPw(), u2.getUserName(), u1.getUserAge(),u1.getUserGender());
		System.out.printf("u3 : %s / %s, / %s, %d / %c\n",
				u3.getUserId(), u3.getUserPw(), u3.getUserName(), u1.getUserAge(),u1.getUserGender());
