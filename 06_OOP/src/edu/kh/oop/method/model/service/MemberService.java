...
public class MemberService {
	...
	public void displayMenu() {
		...
		do {
			...
			case 1 : System.out.println( signUp() ); break;// 만들어뒀던 signup메서드 호출
			// return 값이 스트링(결과)로 반환되니 프린트 구문 안에 적용
	...
	public String login() {
		...
		if(memberId.equals(memberInfo.getMemberId()) &&
				memberPw.equals(memberInfo.getMemberPw()) ) { //memberInfo 객체가 갖고있는 MemberId 를 getter로
			
			// 로그인한 사람의 정보를 기억시키기
			// loginMember에 현재 로그인한 사람의 정보를 기억시키기
			// == loginMember에 memberInfo가 참조하고 있는 객체의 주소 대입
			loginMember = memberInfo;
			// 얕은복사 (같은 객체를 바라보게끔 주소 복사)
			
			// int[] arr = {1,2,3};
			// int[] newArr = arr; 주소값만 복사
	...
	public String selectMember() { //select 조회하다
		System.out.println("\n*******회원 정보 조회*******");
