...
public class MemberService {
	...
	// 속성
	private Scanner sc = new Scanner(System.in); //?스캐너에 private?
	//변수 선언으로 보면 됨
	...
	public void displayMenu() {
		...
		do {
			...
			menuNum = sc.nextInt();
			// 입력 버퍼에 남은 개행문자 제거
			sc.nextLine(); // ?개행문자가 없는 경우에도 상관없는지?
	...
	public int emptyIndex() { //emptyIndex의 역할 정리 시작
	...
	public String signUp() {
		...
		int index = emptyIndex(); // memberArr배열에서 비어있는 인덱스 반환받음
			// ?index로 왜 재대입?	메서드 한번만 쓰기 위해	// 없다면 -1 반환
	...
	public String login() {
		...
		// 2) memberArr 배열 내 요소를 순서대로 접근하여(for문)		
		for(int i = 0; i < memberArr.length; i++) {
			// 2-1) 현재 접근한 요소가 null인지 아닌지 확인(nullPointException피하기 위해)
