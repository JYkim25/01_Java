...
public class ArrayExample1 {
	 ...
	 * - 묶여진 변수들은 하나의 배열명으로 불러지고 구분은 index를 이용함 (index는 0부터 시작하는 정수)
	 * 
	 * */
	...
	public void ex1() {
		...
		// 변수 선언
		int num;
		// stack 영역에 int 자료형을 저장할 수 있는 공간 4byte를 할당하고 그 공간에 num이라는 이름을 부여.
	...
	public void ex2() {
		...
		// 배열의 인덱스 범위를 벗어남 : 길이가 4칸짜리 배열인데, 없는 인덱스번호인 4에 접근했다
		...
		System.out.println( "배열의 길이 : " + arr.length ); //length 는 메서드가 아닌 필드이므로 () 가 없어도 됨
...
Scanner sc = new Scanner(System.in);
		...
		for(int i = 0; i < score.length; i++) {
			...
			// 최고점 비교
			if( score[i] > max) { // 만약에 이렇다면
				max = score[i]; // 갈아끼우기
			}
			
			// 최저점 비교
			if( score[i] < min) { // 만약에 이렇다면
				min = score[i]; // 갈아끼우기
			}
	...
	public void ex6() {
		...
		// flag 상태를 검사
		// flag 가 false 라면 일치하는 값을 찾지 못했다라는 의미
	...
	public void ex8() {
		...
		String str = "가나다" ; // String Constant Pool(문자열 상수 풀)에 저장
		String str2 = new String("가나다"); // Heap 새 객체 저장
		
		System.out.println(str); // 주소값?
		System.out.println(str2);
		System.out.println(str.toString()); //위와 동일
		...
		// 주소값 (객체의 해시코드)을 확인하고 싶다면 아래처럼 사용 (쓸 일은 딱히 없다)
	...
	public void ex9() {
		...
		// sc.next() -> String형
		// String.charAt -> String의 0번째 인덱스 문자 하나를 추출해서 char 형태로 반환
		// "h" -> 'h'
