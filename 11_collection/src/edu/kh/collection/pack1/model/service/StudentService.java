...
public class StudentService {
	...
	// 다형성 중 업캐스팅 적용! (List / ArrayList)
	// 부모 타입(List) 참조변수 (studentList) = 자식 객체(ArrayList) 주소
	...
	public void ex() { // List 테스트용 예제
		...
		for(Object e : testList) {
			...
			if(e instanceof Student) {
				System.out.println(((Student)e).getName());  //??? 괄호 우선순위
	...
	public void selectAll() {
		...
		if(studentList.isEmpty()) {
			...
			return; // 현재 메서드를 종료하고 호출한 곳으로 돌아감. (반환값 없음)
			
		}
		
		// 일반 for 문
		/*
		for(int i = 0; i < studentList.size(); i++) { // 배열이 아니니 length 대신 size
	...
	public String removeStudent() {
		...
		if(studentList.isEmpty()) {
			...
			return "범위를 넘어선 값을 입력할 수 없습니다"; //??? 구문 단순화
			
		} else {
			System.out.println("정말 삭제하시겠습니까? (Y/N)");
			char ch = sc.next().toUpperCase().charAt(0); // charAt 전에 대문자화
			// "y" -> "Y" -> 'y'
	 ...
	 * List를 정렬하는 방법
	 * 
	 * 방법 1 ; Comparable 인터ㅔ페이스를 상속받아 compareTo() 메서드 재정의
	 ...
	 * 익명 내부 클래스 : 이름이 엄ㅎ는 클레스를  즉석에서 선언에서 한번만 사용할 목적으로 작성
